<template>
  <section class="hero-banner h-100">
    <div class="container h-100">
      <div class="row align-items-center h-100">
        <!-- <div class="col-1 d-md-flex d-none justify-content-center align-items-center px-0">
          <HamburgerIcon/>
        </div>-->
        <div class="col-md-7 col-12">
          <h1 class="display-1 hero-banner-title">Jeric Epon</h1>
          <p class="hero-banner-sub-title">Slapping keyboards since 2016</p>
          <p class="hero-banner-body lead">
            Being a developer is my dream job. I've been coding since 2016 and working as a front end developer. I started only from a little knowledge gained form college and now my skill has grown up. But still, there are a lot of stuffs to learn.
          </p>
          <a
            name
            id
            class="btn btn-dark d-flex align-items-center justify-content-center"
            href="/data/CV.pdf"
            role="button"
          >
            <DownloadIcon />
            <span>Gét Résumé</span>
          </a>
        </div>
      </div>
    </div>
  </section>
</template>
<script>
export default {
  name: "HeroBanner"
};
</script>